...
namespace olc {
    ...
    private:
        void DrawBitmap(int x, int y, FT_Bitmap bitmap, olc::Pixel color);
        void DrawBitmapTo(int x, int y, FT_Bitmap bitmap, olc::Pixel color,
                          olc::Sprite *sprite);
...
olc::Font::Font(std::string path, int fontSize) : fontSize{fontSize} {
...
olc::Font &olc::Font::operator=(Font &&other) {
    fontFace = other.fontFace;
    fontSize = other.fontSize;
    fallbacks = std::move(other.fallbacks);
    other.fontFace = nullptr;
...
void olc::Font::Font::DrawString(std::u32string string, int x, int y,
    ...
    for (size_t i = 0; i < string.size(); i++) {
        ...
        if (prevToUse == toUse) {
            FT_Vector kern;
            FT_Get_Kerning(fontFace, string[i - 1], chr, FT_KERNING_DEFAULT,
                           &kern);
        ...
        DrawBitmap(slot->bitmap_left, pge->ScreenHeight() - slot->bitmap_top,
                   bmp, color);
...
olc::FontRect olc::Font::GetStringBounds(std::u32string string, float angle) {
    ...
    for (size_t i = 0; i < string.size(); i++) {
        ...
        FT_Set_Transform(toUse->fontFace, &rotMat, &pen);
        FT_Error error =
                FT_Load_Char(toUse->fontFace, chr, FT_LOAD_BITMAP_METRICS_ONLY);
        if (error) {
            const char *errorString = FT_Error_String(error);
            if (errorString == nullptr) {
                std::cerr
...
olc::Sprite *olc::Font::RenderStringToSprite(std::u32string string,
                                             olc::Pixel color) {
    ...
    for (size_t i = 0; i < string.size(); i++) {
        ...
        FT_Bitmap bmp = toUse->fontFace->glyph->bitmap;
        FT_GlyphSlot slot = toUse->fontFace->glyph;
        DrawBitmapTo(slot->bitmap_left, pge->ScreenHeight() - slot->bitmap_top,
                     bmp, color, sprite);
...
void olc::Font::DrawBitmapTo(int x, int y, FT_Bitmap bmp, olc::Pixel color,
                             olc::Sprite *sprite) {
    switch (bmp.pixel_mode) {
    case FT_PIXEL_MODE_MONO:
        for (size_t bx = 0; bx < bmp.width; bx++) {
...
FT_UInt olc::Font::GetCharIndex(char32_t charCode) {
    return FT_Get_Char_Index(fontFace, charCode);
}

olc::Decal *olc::Font::RenderStringToDecal(std::u32string string,
                                           olc::Pixel color) {
...
olc::Renderable olc::Font::RenderStringToRenderable(std::u32string string,
                                                    olc::Pixel color) {
    Sprite *sprite = RenderStringToSprite(string, color);
...
void olc::Font::AddFallbackFont(std::string path) {
